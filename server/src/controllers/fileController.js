...
/**
 * Return only the owner's protected key material required
 * to create a file share.
 *
 * The server never receives the unwrapped FEK or metadata key.
 */
export async function getFileKeyMaterial(req, res) {
  try {
    ...
    const file = await findFileKeyMaterialForOwner(
      fileId,
      ownerId
    );
    ...
    console.error("File key material error:", error);

    return res.status(500).json({
      ok: false,
      message: "Unable to retrieve protected file key material.",
    });
  }
}
